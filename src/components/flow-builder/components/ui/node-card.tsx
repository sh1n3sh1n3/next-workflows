...
>(({ className, ...props }, ref) => (
  <div
    ref={ref}
    className={cn(
      "w-72 overflow-clip border border-card-foreground/10 rounded-xl bg-card/50 shadow-sm backdrop-blur-xl transition divide-y divide-card-foreground/10 data-[selected=true]:border-primary",
      className
    )}
    {...props}
  />
...
const NodeCardFooter = React.forwardRef<HTMLDivElement, NodeCardFooterProps>(
  ({ className, nodeId, ...props }, ref) => (
    <div
      ref={ref}
      className={`bg-card-foreground/10 px-4 py-2 text-[10px] text-card-foreground/50 ${className}`}
      {...props}
    >
